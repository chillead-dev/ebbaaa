...
function setTokenCookie(res, token){
  ...
  res.setHeader("Set-Cookie", `ex_token=${token}; Path=/; Max-Age=${60*60*24*30}; SameSite=Lax${secure}`);
}
...
export async function signup(req,res,body){
  ...
  await redis.zadd("users",{score:Date.now(),member:uid});
  await redis.del(K.code(email));

  // Favorites (Saved Messages)
  const favId = `fav:${uid}`;
  const favChat = {id:favId,type:"fav",members:[String(uid)],created_at:Date.now()};
  await redis.set(K.chat(favId), JSON.stringify(favChat));
  await redis.set(K.chatLast(favId), JSON.stringify({from_uid:String(uid),text:"",ts:Date.now()}));
  await redis.lpush(K.userChats(uid), favId);
...
export async function login(req,res,body){
  ...
  if(u.blocked) return json(res,403,{error:"blocked"});
