...
export async function list(req,res){
  ...
  for(const id of ids){
    ...
    const pin=safeParse(await redis.get(K.chatPin(id)));
    const unread=Number(await redis.get(K.unread(uid,id))||0);

    let title="Чат"; let peer=null;
    if(chat.type==="fav"){ title="Избранное"; }
...
export async function deleteChat(req,res,body){
  const uid=getUid(req); if(!uid) return json(res,401,{error:"unauthorized"});
  ...
  if(forAll){
    for(const member of chat.members){
      await redis.lrem(K.userChats(member),0,chatId);
      await redis.del(K.unread(member,chatId));
    }
    await redis.del(K.chat(chatId));
...
export async function pinLast(req,res,body){
  const uid=getUid(req); if(!uid) return json(res,401,{error:"unauthorized"});
  const chatId=String(body.chat_id||""); if(!chatId) return json(res,400,{error:"bad_request"});
  const chat=safeParse(await redis.get(K.chat(chatId)));
  if(!chat) return json(res,404,{error:"not_found"});
  ...
  if(!last?.text) return json(res,200,{ok:true});
  await redis.set(K.chatPin(chatId), JSON.stringify({text:last.text, ts:last.ts||Date.now()}));
  return json(res,200,{ok:true});
}
export async function unpin(req,res,body){
  const uid=getUid(req); if(!uid) return json(res,401,{error:"unauthorized"});
  const chatId=String(body.chat_id||""); if(!chatId) return json(res,400,{error:"bad_request"});
  const chat=safeParse(await redis.get(K.chat(chatId)));
  if(!chat) return json(res,404,{error:"not_found"});
  if(!chat.members?.includes(String(uid))) return json(res,403,{error:"forbidden"});
  await redis.del(K.chatPin(chatId));
  return json(res,200,{ok:true});
}
export async function clearMe(req,res,body){
  const uid=getUid(req); if(!uid) return json(res,401,{error:"unauthorized"});
  const chatId=String(body.chat_id||""); if(!chatId) return json(res,400,{error:"bad_request"});
  await redis.lrem(K.userChats(uid), 0, chatId);
  await redis.set(K.unread(uid,chatId), "0");
...
export async function clearAll(req,res,body){
  const uid=getUid(req); if(!uid) return json(res,401,{error:"unauthorized"});
  const chatId=String(body.chat_id||""); if(!chatId) return json(res,400,{error:"bad_request"});
  const chat=safeParse(await redis.get(K.chat(chatId)));
  if(!chat) return json(res,404,{error:"not_found"});
  if(!chat.members?.includes(String(uid))) return json(res,403,{error:"forbidden"});
  await redis.del(K.chatMsgs(chatId));
  await redis.set(K.chatLast(chatId), JSON.stringify({from_uid:String(uid),text:"",ts:Date.now()}));
  await redis.del(K.chatPin(chatId));
  for(const m of (chat.members||[])){
    await redis.set(K.unread(m,chatId), "0");
  }
  return json(res,200,{ok:true});
}
export async function deleteMe(req,res,body){
  return clearMe(req,res,body);
}
export async function deleteAll(req,res,body){
  const uid=getUid(req); if(!uid) return json(res,401,{error:"unauthorized"});
  const chatId=String(body.chat_id||""); if(!chatId) return json(res,400,{error:"bad_request"});
  const chat=safeParse(await redis.get(K.chat(chatId)));
  if(!chat) return json(res,404,{error:"not_found"});
  if(!chat.members?.includes(String(uid))) return json(res,403,{error:"forbidden"});
  // remove from members lists
  for(const m of (chat.members||[])){
    await redis.lrem(K.userChats(m), 0, chatId);
    await redis.del(K.unread(m,chatId));
  }
  await redis.del(K.chatMsgs(chatId));
  await redis.del(K.chatLast(chatId));
  await redis.del(K.chatPin(chatId));
