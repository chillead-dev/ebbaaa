...
function vBadgeHtml(u){
  if(!u?.verified) return "";
  return `<span class="vbadge" title="Verified"><svg viewBox="0 0 24 24"><path d="M12 2l3 3 4 .5-1.5 3.7 1.5 3.7-4 .5-3 3-3-3-4-.5 1.5-3.7L5 5.5 9 5l3-3z" fill="currentColor" opacity=".28"/><path d="M9.2 12.2l1.7 1.7 4.1-4.1" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/></svg></span>`;
}
...
export async function loadChats(){
  ...
  if(v) v.innerHTML = `<div class="list">${Array.from({length:6}).map(()=>`<div class="row skel" style="height:76px"></div>`).join('')}</div>`;
  ...
  for(const it of list){
    if(it.k==="u"){
      ...
      html+=`<div class="row" data-user="${esc(u.username)}">
        ...
        <div class="mid"><div class="title">${esc(u.display_name||u.username)} ${vBadgeHtml(u)}</div><div class="sub">@${esc(u.username)}</div></div>
        <div class="right"><div class="time"></div></div>
      </div>`;
    }else{
      const c=it.c;
      html+=`<div class="row" data-chat="${esc(c.id)}">
        ...
        <div class="mid"><div class="title">${esc(c.title)} ${c.peer? vBadgeHtml(c.peer):""}</div><div class="sub">${esc(c.last_text||"Сообщений пока нет")}</div></div>
